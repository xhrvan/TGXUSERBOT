...
LEGENDBOT = Config.STICKER_PACKNAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"ForGo10God_LEGENDBOT_{pack}"
        packnick = (
            f"{LEGENDBOT} Vol.{pack}"
            if LEGENDBOT
            else f"@ForGo10God's LEGENDBOT Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            async with bot.conversation("Stickers") as conv:
                ...
                while "120" in x.text:
                    pack += 1
                    packname = f"ForGo10God_by_ForGo10God_{pack}"
                    packnick = (
                        f"{LEGENDBOT} Vol.{pack}"
                        if LEGENDBOT
                        else f"@ForGo10God's LEGENDBOT Vol.{pack}"
            ...
            await event.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(admin_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(admin_cmd(pattern=r"editst ?(.*)", outgoing=True))
