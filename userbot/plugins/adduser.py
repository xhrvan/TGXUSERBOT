...
@borg.on(admin_cmd(pattern="edds ?(.*)"))
@bot.on(admin_cmd(pattern="edds$", outgoing=True))
@bot.on(sudo_cmd(pattern="edds$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CmdHelp("add").add_command(
  "edd", None, "Add To Member To Ur Group."
).add()
