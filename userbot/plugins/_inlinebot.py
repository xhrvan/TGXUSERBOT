...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@Legend_Mr_Hacker":
            rev_text = query[::-1]
            veriler = button(0, sorted(CMD_HELP))
            result = await builder.article(
                f"Hey! Only use .help please",
                text=f"**RUNNING AF** [LEGENDBOT](https://t.me/Legend_Mr_Hacker)\n\n**Number of modules installed :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False,
            )
            ...
            result = builder.article(
                ...
                buttons=[
                    [
                        custom.Button.url("🔥 LEGEND 🔥", "https://t.me/Legend_Mr_Hacker"),
                        custom.Button.url(
    ...
    async def Information(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "Mil Gyi Tasalli..? Kabse mere bot me ungli kr rhe h. Khudka bna lo na agr chaiye to pta nhi kaha se aajate h disturb krne. [©LEGENDBOT](https://t.me/Legend_Mr_Hacker) ",
                cache_time=0,
                alert=True,
            )
