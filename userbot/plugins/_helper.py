...
async def yardim(event):
    ...
    if tgbotusername is not None or LEGEND_input == "text":
        results = await event.client.inline_query(tgbotusername, "@Tgxbotz_Chat")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of TGXBOT\n\n"
        )
        LEGENDcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the TGXBOT are [here]({url})"
