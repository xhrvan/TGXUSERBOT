...
**DESCRIPTION** : `{description}`
  '''
    ...
    if not event.query.user_id == bot.uid:
        resultm = builder.article(title="me not your bot",description="Mind Your Business",text="Hey U Must Use https://github.com/LEGEND-OS/LEGENDBOT ",buttons=[[Button.switch_inline("Search Again", query="pokedex ", same_peer=True)],], )
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id == bot.uid:
        ...
        await event.answer([result])
    if not event.query.user_id == bot.uid:
        resultm = builder.article(title="me not your bot",description="Mind Your Business",text="Hey U Must Use https://github.com/LEGEND-OS/LEGENDBOT  ",buttons=[[Button.switch_inline("Search Again", query="pokecard ", same_peer=True)],], )
        await event.answer([resultm])
        return
CmdHelp("dcinlindex").add_command(
  'pokedex', None, 'Use and See'
).add()
